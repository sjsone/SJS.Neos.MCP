...
### Re-Evaluate the Connection concept

Renamed from "Agent" to "Connection" — a Connection is a Bearer-Token-based client identity. Future concepts of "doing things on behalf" will be layered on top.
...
Currently the Connection works on behalf of the Neos _Backend_ User with its full authorization. This is not ideal.

For the Neos Backend User Module has to be able to select what _Roles_ the connection should inherit from the User.
